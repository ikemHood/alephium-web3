...
describe('contract', function () {
  ...
  it('should test inline functions(unit test)', async () => {
    const contractAddress = randomContractAddress()
    const result0 = await InlineTest.tests.nextCount_({
    ...
    const result1 = await InlineTest.tests.nextCount({
      address: contractAddress,
      initialFields: { count: 0n },
      inputAssets: [{ address: signer.address, asset: { alphAmount: ONE_ALPH } }],
      initialAsset: { alphAmount: ONE_ALPH }
    ...
    expect(assets1.alphAmount).toEqual(ONE_ALPH + ONE_ALPH / 100n)
  })
  ...
  it('should test inline functions(integration test)', async () => {
    ...
    await instance.transact.nextCount_({ signer, attoAlphAmount: ONE_ALPH })
    ...
    expect(state1.asset.alphAmount).toEqual(ONE_ALPH + (ONE_ALPH / 100n) * 2n)
  })
})
